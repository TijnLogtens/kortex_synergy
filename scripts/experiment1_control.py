...
class Experiment1Control:
    def __init__(self):
        try:
            ...
            # Track and define pose (Home ~= 0.4,0,04,0,0,0)
            self.pose = {
                'x' : 0,
                'y' : 0,
                'z' : 0,
    ...
    def pose_callback(self, data):
        ...
        # Apply the selected paradigm transformation
        if self.paradigm == 1:
    ...
    def main(self):
        ...
        if success:
            ...
            # Wait for the paradigm to be received
            while self.paradigm is None and not rospy.is_shutdown():
                rospy.loginfo("Waiting for paradigm...")
                rospy.sleep(1)
